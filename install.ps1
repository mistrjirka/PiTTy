...
try {
  ...
  $Missing = @(); if (-not $PiList.Contains("github.com/mistrjirka/pi-subagent")) { $Missing += "@mistrjirka/pi-subagent" }; if (-not $PiList.Contains("@juicesharp/rpiv-todo")) { $Missing += "@juicesharp/rpiv-todo" }; if (-not $PiList.Contains("pi-mcp-adapter")) { $Missing += "pi-mcp-adapter" }; if (-not $PiList.Contains("github.com/mistrjirka/pi-one-round-compaction")) { $Missing += "pi-one-round-compaction" }
  ...
  $Failures = 0
  foreach ($Plugin in @(@("git:github.com/mistrjirka/pi-subagent", "github.com/mistrjirka/pi-subagent", "pi-subagents"), @("npm:@juicesharp/rpiv-todo", "@juicesharp/rpiv-todo", ""), @("npm:pi-mcp-adapter", "pi-mcp-adapter", ""), @("git:github.com/mistrjirka/pi-one-round-compaction", "github.com/mistrjirka/pi-one-round-compaction", "pi-smart-compact"))) { if ($PluginMode -eq "yes" -and -not $PiList.Contains($Plugin[1])) { if ($Plugin[2] -ne "" -and ($PiList.Contains($Plugin[2]) -or (& pi list 2>$null | Out-String).Contains($Plugin[2]))) { Write-Host "Skipping $($Plugin[0]): $($Plugin[2]) is already installed" } else { $Log = Join-Path $LogRoot ("plugin-" + $Plugin[1].Replace("@", "_").Replace("/", "_") + ".log"); & pi install $Plugin[0] *> $Log; if ($LASTEXITCODE -ne 0) { $Failures++; Write-Warning "Optional Pi package $($Plugin[0]) failed (exit code: $LASTEXITCODE); log: $Log" } } } }
