...
try {
  ...
  $Missing = @(); if (-not $PiList.Contains("pi-subagents")) { $Missing += "pi-subagents" }; if (-not $PiList.Contains("@juicesharp/rpiv-todo")) { $Missing += "@juicesharp/rpiv-todo" }; if (-not $PiList.Contains("pi-mcp-adapter")) { $Missing += "pi-mcp-adapter" }; if (-not $PiList.Contains("pi-smart-compact")) { $Missing += "pi-smart-compact" }
  ...
  $Failures = 0
  foreach ($Plugin in @(@("npm:pi-subagents", "pi-subagents"), @("npm:@juicesharp/rpiv-todo", "@juicesharp/rpiv-todo"), @("npm:pi-mcp-adapter", "pi-mcp-adapter"), @("npm:pi-smart-compact", "pi-smart-compact"))) { if ($PluginMode -eq "yes" -and -not $PiList.Contains($Plugin[1])) { $Log = Join-Path $LogRoot ("plugin-" + $Plugin[1].Replace("@", "_").Replace("/", "_") + ".log"); & pi install $Plugin[0] *> $Log; if ($LASTEXITCODE -ne 0) { $Failures++; Write-Warning "Optional Pi package $($Plugin[0]) failed (exit code: $LASTEXITCODE); log: $Log" } } }
