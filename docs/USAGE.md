...
Top-level subagent trees are shown newest-run-first. Recursive descendants stay directly under their actual parent in spawn order, at any depth; activity updates do not reshuffle the tree. The sidebar, selector, keyboard cycling, and fallback selection all use the same tree order.
