...
describe("PowerShell installer executable fixture", () => {
  test.skipIf(process.platform !== "win32")("runs isolated staging, plugin, checksum, and rollback paths", async () => {
    ...
    const env = { ...process.env, PATH: `${fake};${process.env.PATH}`, PITTY_LOG_DIR: path.join(base, "logs") };
    ...
    expect(direct.code).toBe(0); expect(direct.output).toContain("already installed"); expect(direct.output).not.toContain("pi install");
