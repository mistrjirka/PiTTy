...
import { filterThemeTokens, ThemePresetBrowser, ThemeTokenEditor } from "../src/ui/theme-settings.tsx";
...
import { McpSettings, type McpConfigState, type McpInstallState, type McpSettingsView } from "../src/ui/mcp-settings.tsx";
...
async function mount(node: () => JSX.Element, width = 100, height = 30): Promise<TestRendererSetup> {
  const setup = await testRender(node, { width, height, useMouse: true, enableMouseMovement: true });
  active.push(setup);
  await setup.flush();
...
function descriptors(
  ...
  return [
    { id: "model", label: "Model", summary: () => "provider/model", disabledReason: () => undefined, busy, inlineError: error, open: () => opened.push("model") },
    { id: "thinking", label: "Thinking effort", summary: () => "high", disabledReason: () => undefined, busy, inlineError: error, open: () => opened.push("thinking") },
    { id: "session", label: "Session", summary: () => "Current session", disabledReason: () => undefined, busy, inlineError: error, open: () => opened.push("session") },
  ];
}
...
describe("Settings components", () => {
  test("renders ordered descriptors and opens the selected row", async () => {
    ...
    const setup = await mount(() => (
      <SettingsHub descriptors={() => descriptors(opened)} selectedId={() => "model"} onSelected={() => {}} onClose={() => {}} />
    ));
    const frame = setup.captureCharFrame();
    expect(frame.indexOf("Model")).toBeLessThan(frame.indexOf("Thinking effort"));
    expect(frame.indexOf("Thinking effort")).toBeLessThan(frame.indexOf("Session"));
  ...
  test("reports and restores the originating selected row", async () => {
    ...
    const restored = await mount(() => (
      <SettingsHub descriptors={() => descriptors([])} selectedId={() => selected.at(-1) ?? "model"} onSelected={() => {}} onClose={() => {}} />
    ));
    const select = restored.renderer.root.findDescendantById("settings-sections") as SelectRenderable | undefined;
  ...
  test("RPC-unavailable rows show a reason and reject keyboard and mouse activation", async () => {
    const opened: string[] = [];
    const rows = descriptors(opened).map((row) => ({ ...row, disabledReason: () => "Pi RPC unavailable" }));
    const setup = await mount(() => (
      <SettingsHub descriptors={() => rows} selectedId={() => "model"} onSelected={() => {}} onClose={() => {}} />
    ));
  ...
  test("reactively renders one row busy and exposes inline errors", async () => {
    ...
    const setup = await mount(() => (
      <SettingsHub descriptors={() => descriptors([], busy, error)} selectedId={() => "model"} onSelected={() => {}} onClose={() => {}} />
    ));
    setBusy(true);
    setError("failed");
    await setup.flush();
  ...
  test("root close and nested back routes are explicit", async () => {
    let closed = 0;
    const setup = await mount(() => (
      <SettingsHub descriptors={() => descriptors([])} selectedId={() => "model"} onSelected={() => {}} onClose={() => { closed += 1; }} />
    ));
  ...
  test("thinking effort supports keyboard and mouse selection", async () => {
    ...
    const keyboard = await mount(() => (
      <ThinkingSelector current="medium" onSelect={(level) => selected.push(level)} onCancel={() => {}} />
    ), 80, 24);
  ...
  test("rename is prefilled, rejects empty input, and confirms a replacement", async () => {
    ...
    let cancelled = 0;
    const setup = await mount(() => (
      <SessionRename initialName="Prior" onConfirm={(name) => names.push(name)} onCancel={() => { cancelled += 1; }} />
    ));
    const editor = setup.renderer.root.findDescendantById("settings-session-rename") as TextareaRenderable | undefined;
  ...
  test("rename busy state blocks duplicates and a failure updates in place", async () => {
    ...
    const names: string[] = [];
    const setup = await mount(() => (
      <SessionRename initialName="Prior" busy={busy} error={error} onConfirm={(name) => names.push(name)} onCancel={() => {}} />
    ));
    setup.mockInput.pressEnter();
    await setup.flush();
    expect(names).toEqual([]);
  ...
  test("theme token editor searches every token and immediately applies complete valid colors", async () => {
    ...
    setup.mockInput.pressEnter();
    await setup.flush();
    const editor = setup.renderer.root.findDescendantById("theme-token-value") as TextareaRenderable | undefined;
    expect(editor?.plainText).toBe(colors.toolReadBg);
    editor?.clear();
    await setup.mockInput.typeText("#12345");
    await setup.flush();
    expect(changes).toEqual([]);
    expect(setup.captureCharFrame()).toContain("complete color in #RRGGBB form");
    ...
    for (const token of themeColorTokens) expect(filterThemeTokens(token)).toContain(token);
  });
  ...
  test("theme token editor reports live low contrast and exposes mouse reset", async () => {
    const lowContrastPalette = { ...themePresets["PiTTy Midnight"], text: "#09090B", muted: "#09090B" };
    let resets = 0;
    const setup = await mount(() => (
      <ThemeTokenEditor
        preset="PiTTy Midnight"
  ...
  test("MCP settings renders install, scope, preview, and activation states without custom panels", async () => {
    ...
    const snapshot = createSignal<McpConfigSnapshot>({ scope: "global", filePath: "/tmp/mcp.json", exists: true, sourceFingerprint: "x", document: { mcpServers: {} }, servers: {} });
    const view = createSignal<McpSettingsView>("list");
    const setup = await mount(() => <McpSettings adapterState={install[0]} configState={config[0]} scope={scope[0]} snapshot={snapshot[0]} preview={() => ({ snapshot: snapshot[0](), mutation: { kind: "delete", name: "demo" }, before: { command: "old" }, after: undefined, document: { mcpServers: {} }, warnings: ["Configured command executes with the user's permissions."] })} error={() => undefined} view={view[0]} onViewChange={view[1]} onInstall={() => install[1]("installing")} onScope={scope[1]} onAdd={() => {}} onDelete={() => {}} onConfirmPreview={() => {}} onCancelPreview={() => {}} onRetryActivation={() => {}} onBack={() => {}} />);
    const frame = setup.captureCharFrame();
    ...
    expect(frame).toContain("Global changes affect other MCP hosts."); expect(frame).toContain("Before/after preview"); expect(frame).toContain("never invokes /mcp");
  });

  test("MCP settings uses a list-first flow for empty and populated scopes", async () => {
    const emptySnapshot: McpConfigSnapshot = { scope: "global", filePath: "/tmp/home/.config/mcp/mcp.json", exists: false, sourceFingerprint: "empty", document: { mcpServers: {} }, servers: {} };
    const populatedSnapshot: McpConfigSnapshot = { ...emptySnapshot, exists: true, sourceFingerprint: "populated", document: { mcpServers: { demo: { command: "node", args: ["server.js"] }, remote: { url: "https://example.test/mcp" } } }, servers: { demo: { command: "node", args: ["server.js"] }, remote: { url: "https://example.test/mcp" } }, laterPrecedenceNames: ["remote"] };
    const view = createSignal<McpSettingsView>("list");
    const common = {
      ...
      preview: () => undefined,
      error: () => undefined,
      view: view[0], onViewChange: view[1],
      onInstall: () => {}, onScope: () => {}, onAdd: () => {}, onDelete: () => {}, onConfirmPreview: () => {}, onCancelPreview: () => {}, onRetryActivation: () => {}, onBack: () => {},
    };
    const empty = await mount(() => <McpSettings {...common} view={view[0]} onViewChange={view[1]} snapshot={() => emptySnapshot} />);
    ...
    const addForm = await mount(() => <McpSettings {...common} view={() => "form"} onViewChange={view[1]} snapshot={() => emptySnapshot} />);
    expect(addForm.captureCharFrame()).toContain("Command (required)");
    expect(addForm.captureCharFrame()).toContain("Cancel");
    const populated = await mount(() => <McpSettings {...common} view={view[0]} onViewChange={view[1]} snapshot={() => populatedSnapshot} />);
