...
export function reconcilePendingSteers(
	...
	for (const run of runs) {
		...
		if (
			!existing ||
			run.active ||
			(run.steerCount ?? 0) > (existing.steerCount ?? 0)
		) {
			byRun.set(run.runId, {
				...run,
	...
	for (const run of byRun.values()) {
		if (run.steerCount !== undefined) {
			...
			const baseline =
				runEntries.length > 0
					? Math.min(...runEntries.map((entry) => entry.baselineSteerCount))
					: run.steerCount;
	...
	return entries.filter((entry) => {
		...
		if (
			remaining > 0 &&
			run.steerCount !== undefined &&
			run.steerCount > entry.baselineSteerCount
		) {
...
export type SettledDispatchDecision =
	| "dispatch"
	| "suppressed"
	| "busy"
	| "empty";
...
export function settledDispatchDecision(
	gate: DispatchGate,
	hasItems: boolean,
	streaming: boolean,
...
export function clearTargetDraft(
...
export function snapshotQueue(
	items: readonly LocalQueuedMessage[],
): QueueDispatchSnapshot {
	const snapshot = [...items];
	return {
		items: snapshot,
		prompt: snapshot.map((item) => item.text.trim()).join("\n\n"),
	};
}

export function restoreQueue(
	snapshot: readonly LocalQueuedMessage[],
	current: readonly LocalQueuedMessage[],
): LocalQueuedMessage[] {
