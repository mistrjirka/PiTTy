...
export async function runUpgrade(args: readonly string[], environment: NodeJS.ProcessEnv = process.env): Promise<number> {
  ...
  compareStableVersions(version, metadata.installedVersion);
  const script = path.join(installDirectory, "install.sh");
  const result = await new Promise<number>((resolve, reject) => {
    const child = spawn("sh", [script, "--defer-apply", "--version", version, "--install-dir", metadata.installDirectory, "--bin-dir", metadata.binDirectory, "--repo", metadata.repository], { stdio: "inherit", env: { ...environment, PITTY_PLUGIN_MODE: metadata.pluginMode } });
