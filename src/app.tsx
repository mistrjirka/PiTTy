...
import {
	McpSettings,
	type McpConfigState,
	type McpFormConfirmations,
	type McpInstallState,
	type McpSettingsView,
} from "./ui/mcp-settings.tsx";
import {
...
} from "./ui/list-stability.ts";
import {
	detectOptionalIntegrations,
...
export function settingsBackRoute(route: SettingsRoute): SettingsRoute {
	...
	if (route === "session-switch" || route === "session-rename")
		return "session";
...
export function subagentInspectDecision(
	inspectorOpen: boolean,
	targetCount: number,
...
const localCommandChoices: CommandChoice[] = [
	...
	{
		name: "settings",
		description: "Show current UI and Pi settings",
		source: "ui",
	},
	{
		name: "status",
	...
	{
		name: "sidebar",
		description: "Enable or disable the sidebar",
		source: "ui",
	},
	{
		name: "autocompact",
		description: "Enable or disable automatic compaction",
		source: "ui",
	},
	{
		name: "retry",
	...
	{
		name: "steering-mode",
		description: "Set steering delivery mode",
		source: "ui",
	},
	{
		name: "followup-mode",
...
function isExtensionUiRequest(value: unknown): value is RpcExtensionUIRequest {
	return Boolean(
		value &&
			typeof value === "object" &&
			(value as Record<string, unknown>).type === "extension_ui_request",
	);
}
...
function keyIs(
	...
	return (
		...
		(modifiers.shift === undefined ||
			Boolean(event.shift) === modifiers.shift) &&
...
export type StreamingCtrlCDecision = "abort" | "force-exit";

export function streamingCtrlCDecision(
	abortInFlight: boolean,
): StreamingCtrlCDecision {
	return abortInFlight ? "force-exit" : "abort";
}
...
export function App(props: AppOptions) {
	...
	const [mcpInstallState, setMcpInstallState] = createSignal<McpInstallState>(
		props.integrations.mcpAdapter.installed ? "ready" : "missing",
	);
	...
	const [settingsBusy, setSettingsBusy] = createSignal<
		"model" | "thinking" | "rename" | "theme"
	>();
	const [settingsErrors, setSettingsErrors] = createSignal<
		Partial<Record<"model" | "thinking" | "rename" | "theme", string>>
	>({});
	const [themeConfig, setThemeConfig] = createSignal<ThemeConfigDocument>(
		props.themeConfig ?? {
			version: 1,
			theme: { preset: "PiTTy Midnight", overrides: {} },
		},
	);
	const themeController = props.themeController ?? createThemeController();
	let settingsOrigin: Renderable | null = null;
	const [sessionDiscovery, setSessionDiscovery] =
		createSignal<SessionDiscoveryState>({ kind: "loading" });
	...
	const [pendingSessionReason, setPendingSessionReason] = createSignal<
		"streaming" | "queued"
	>();
	...
	const inspectSubagent = () => { revision(); return activeRuntime().inspectSubagent; };
	const forkingConversation = () => { revision(); return activeRuntime().forkInProgress; };
	const displayStatus = () => {
		if (forkingConversation()) return "forking conversation…";
		const current = status();
		if (current !== "forking conversation…") return current;
		return streaming() ? "working" : "ready";
	};
	const setSelectedTargetKey = (key: string | undefined) => {
		const runtime = activeRuntime();
		if (runtime.selectedTargetKey === key) return;
		if (key === undefined) delete runtime.selectedTargetKey;
		else runtime.selectedTargetKey = key;
		touch();
	};
	const setInspectSubagent = (value: boolean) => {
		const runtime = activeRuntime();
		if (runtime.inspectSubagent === value) return;
		runtime.inspectSubagent = value;
		touch();
	};
	...
	const handleExtensionRequest = (envelope: ExtensionRequestEnvelope): void => {
		...
		if (!runtime || activeTabId() !== envelope.runtimeId) return;
	...
	const forkAt = async (entryId: string, userItem?: ConversationItem | undefined) => {
		...
		if (!plan.canCreate || sourceRuntime.forkInProgress) {
			toast(plan.canCreate ? "A fork is already being prepared." : "Maximum of 8 conversation tabs.", "warning");
			return;
		}
		sourceRuntime.forkInProgress = true;
		touch();
		setStatus("forking conversation…");
		const provisionalId = `fork-${++tabCounter}`;
		const provisional = createTabRuntime({ id: provisionalId, cwd: props.cwd, args: props.piArgs, ...(props.piExecutable ? { executable: props.piExecutable } : {}), sessionFile: originalSessionFile, logger: props.logger,
			onEvent: (event, runtime) => {
				if (runtimeMap().has(runtime.id)) {
					routeRuntimeEvent(event, runtime);
					return;
				}
				if (isExtensionUiRequest(event))
		...
		provisionalRuntimes.add(provisional);
		try {
			...
			const finalPlan = planForkTab(
				originalSessionFile,
				branchedState.sessionFile,
				tabState().tabs.length,
			);
			if (!finalPlan.canCreate) throw new Error("Maximum of 8 conversation tabs.");
			if (!tabManager.create({ id: provisionalId, badges: 0 })) throw new Error("Maximum of 8 conversation tabs.");
			provisionalRuntimes.delete(provisional);
			setRuntimeMap((current) => new Map(current).set(provisionalId, provisional));
			refreshTabs();
			activateTab(provisionalId);
			setStatus(branchedState.isStreaming ? "working" : "ready");
			toast("Fork created.", "success");
		} catch (error) {
			provisionalRuntimes.delete(provisional);
			extensionRouter.remove(provisionalId);
			provisional.messageUpdates.dispose();
			provisional.coalescer.flush();
			try {
				await provisional.client.stop();
			} catch (stopError) {
				props.logger.warn("tabs.provisional_stop_failed", stopError);
			}
			toast(error instanceof Error ? error.message : String(error), "error", 5000);
		} finally {
			sourceRuntime.forkInProgress = false;
			if (tabRuntimeIsActive(sourceRuntime.id)) touch();
			setStatus(activeRuntime().conversation.isStreaming ? "working" : "ready");
		}
	};
	...
	const tabRuntimeIsActive = (id: string) => activeTabId() === id;
	const loadRuntimeHistory = async (runtime: ConversationTabRuntime) => {
		const before = [...runtime.conversation.items];
		const messages = await runtime.client.getMessages(SESSION_LOAD_REQUEST_TIMEOUT_MS);
		const liveConversationChanged =
			runtime.conversation.items.length !== before.length ||
			runtime.conversation.items.some((item, index) => item !== before[index]);
		if (liveConversationChanged) {
			props.logger.warn("tabs.history_load_raced_live_events", { runtimeId: runtime.id });
			return;
		}
		...
		if (tabRuntimeIsActive(runtime.id)) { resetMessageWindow(runtime.conversation.items.length); conversationTouch(); }
	};
	...
	const conversationTouch = () => setConversationRevision((value) => value + 1);
	const visibleSidebar = createMemo(
		() => sidebarEnabled() && dimensions().width >= 104,
	);
	...
	const todos = createMemo<TodoViewItem[]>((previous) =>
		preserveEquivalentTodos(previous, deriveTodos(items())),
	);
	const subagentsAvailable = createMemo(
		() => props.integrations.subagents.installed || runs().length > 0,
	);
	const todosAvailable = createMemo(
	...
	const subagentTools = createMemo<ToolItem[]>((previous) =>
		preserveReferencedList(
			previous,
			items().filter((item): item is ToolItem => item.kind === "tool"),
		),
	);
	...
	const toast = (
		...
		setNotificationHistory((current) =>
			appendNotificationHistory(current, record),
		);
	...
	const updateScrollPosition = () => {
		...
		const viewportHeight = Math.max(
			1,
			scroll.height || dimensions().height - 8,
		);
	...
	const restoreThemeViewport = (snapshot: ThemeViewportSnapshot) => {
		queueMicrotask(() =>
			queueMicrotask(() =>
				setTimeout(() => {
					if (snapshot.mainTop !== undefined)
						scroll?.scrollTo({ x: 0, y: snapshot.mainTop });
					if (snapshot.subagentTop !== undefined)
						subagentScroll?.scrollTo({ x: 0, y: snapshot.subagentTop });
	...
	const loadOlderMessages = () => {
		...
		setMessageWindowStart(
			loadOlderMessageWindowStart(current, MESSAGE_LOAD_BATCH),
		);
		...
		switch (name) {
			...
			case "thinking": {
				...
				if (!isThinkingLevel(argument)) {
					addSystem(
						"Usage: /thinking minimal|low|medium|high|xhigh|max",
						"warning",
					);
					return true;
				}
			...
			case "models": {
				...
				addSystem(
					`Available models (${labels.length}):\n${labels.slice(0, 80).join("\n")}${labels.length > 80 ? "\n…" : ""}`,
				);
				return true;
			}
			case "compact":
			...
			case "commands": {
				...
				addSystem(
					rows.length
						? `Pi extension/template/skill commands:\n${rows.join("\n")}`
			...
			case "thoughts": {
				...
				addSystem(
					`Thinking blocks ${thinkingExpanded() ? "expanded" : "collapsed"} by default. Individual blocks remain clickable.`,
					"success",
				);
				return true;
			}
	...
	const closeSettings = () => {
		...
		settingsOrigin = null;
		queueMicrotask(() => {
			if (origin && !origin.isDestroyed) origin.focus();
			else focusMainPrompt();
		});
	};
	const openNotification = (recordId: string) => {
		setNotificationHistory((current) =>
			current.map((record) =>
				record.id === recordId ? { ...record, read: true } : record,
			),
		);
		setNotificationDetailId(recordId);
	};
	...
	const loadModels = async (fromSettings: boolean): Promise<boolean> => {
		...
		try {
			...
			if (fromSettings) setSettingsBusy(undefined);
		}
	};
	...
	const applySettingsThinking = async (level: ThinkingLevel) => {
		...
		try {
			...
			setSettingsErrors((errors) => ({
				...errors,
				thinking: error instanceof Error ? error.message : String(error),
			}));
		} finally {
			setSettingsBusy(undefined);
		}
	...
	const applyThemeDocument = async (document: ThemeConfigDocument) => {
		...
		const pendingCommit = commitThemeDocument(
			themeController,
			previous,
			document,
			writeThemeConfig,
	...
	const applyThemePreset = (preset: ThemePresetName) =>
		void applyThemeDocument({ version: 1, theme: { preset, overrides: {} } });
	const applyThemeToken = (token: keyof ThemePalette, value: string) =>
		void applyThemeDocument({
			version: 1,
			theme: {
	...
	const resetTheme = () =>
		void applyThemeDocument({
			version: 1,
			theme: { preset: themeConfig().theme.preset, overrides: {} },
		});
	...
	const applySettingsRename = async (name: string) => {
		...
		try {
			...
			setSettingsErrors((errors) => ({
				...errors,
				rename: error instanceof Error ? error.message : String(error),
			}));
		} finally {
			setSettingsBusy(undefined);
		}
	...
	const openMcpSettings = () => {
		const snapshot = loadMcpConfig(mcpScope(), props.cwd);
		setMcpView("list");
		...
		setMcpConfigState(
			snapshot.readOnlyReason
				? "malformed-readonly"
				: Object.keys(snapshot.servers).length
					? "loaded"
					: "empty",
		);
	...
	const installMcpAdapter = () => {
		...
		void installOptionalIntegrationAsync("pi-mcp-adapter", {
			...(props.piExecutable ? { piExecutable: props.piExecutable } : {}),
			cwd: props.cwd,
		})
			.then(async (result) => {
				if (!result.ok) {
					setMcpInstallState("install-failed");
					setMcpError(
						`Failed to install ${result.packageName}; command ${result.command.join(" ")}; exit ${result.exitCode ?? "unknown"}; log ${result.logPath}`,
					);
					return;
				}
				setMcpInstallState("ready");
				if (streaming()) {
					setMcpActivationPending(true);
					setMcpConfigState("saved-pending-busy");
				} else await activateMcp();
			})
			.catch((error: unknown) => {
				setMcpInstallState("install-failed");
				setMcpError(error instanceof Error ? error.message : String(error));
	...
	const prepareMcpMutation = (
		form: McpServerForm,
		confirmations: McpFormConfirmations = {},
	) => {
		...
		try {
			setMcpPreview(
				previewMcpMutation(
					snapshot,
					{ kind: "upsert", server: form },
					confirmations.confirmPlaintext === true,
					confirmations.confirmDuplicate === true,
				),
			);
			setMcpConfigState("mutation-preview");
			setMcpError(undefined);
		} catch (error) {
			setMcpError(error instanceof Error ? error.message : String(error));
		}
	...
	const deleteMcpServer = (name: string) => {
		...
		if (!snapshot) return;
		try {
			setMcpPreview(previewMcpMutation(snapshot, { kind: "delete", name }));
			setMcpConfigState("mutation-preview");
		} catch (error) {
			setMcpError(error instanceof Error ? error.message : String(error));
		}
	};
	const confirmMcpMutation = () => {
		const preview = mcpPreview();
		if (!preview) return;
		void commitMcpMutation(preview)
			.then(async () => {
				setMcpSnapshot(loadMcpConfig(mcpScope(), props.cwd));
				setMcpPreview(undefined);
				if (streaming()) {
					setMcpActivationPending(true);
					setMcpConfigState("saved-pending-busy");
		...
		return [
			{
				...
				busy: () => settingsBusy() === "model",
				inlineError: () => settingsErrors().model,
				open: () => {
					if (!unavailable) {
			...
			},
			{
				...
				summary: () => state?.thinkingLevel ?? "Unavailable",
				disabledReason: () => unavailable,
				busy: () => settingsBusy() === "thinking",
				inlineError: () => settingsErrors().thinking,
				open: () => {
					if (!unavailable) setSettingsRoute("thinking");
				},
			},
			{
				id: "session",
				label: "Session",
				summary: () => state?.sessionName || "Unnamed",
				disabledReason: () => unavailable,
				busy: () => false,
				inlineError: () => settingsErrors().rename,
				open: () => {
					if (!unavailable) setSettingsRoute("session");
				},
			},
			{
				id: "theme",
				label: "Theme",
				summary: () => themeConfig().theme.preset,
				disabledReason: () => undefined,
				busy: () => settingsBusy() === "theme",
				inlineError: () => settingsErrors().theme,
				open: () => setSettingsRoute("theme"),
			},
			{
				id: "mcp",
				label: "MCP servers",
				summary: () =>
					mcpInstallState() === "ready" ? "Adapter ready" : "Adapter optional",
				disabledReason: () => undefined,
				busy: () => mcpInstallState() === "installing",
	...
	const discoverCurrentSessions = async () => {
		...
		try {
			const choices = await discoverSessions(
				props.cwd,
				sessionState()?.sessionFile,
				(loaded, total) => {
					if (generation === sessionDiscoveryGeneration)
						setSessionDiscovery({
							kind: "loading",
	...
	const switchToSession = async (choice: SessionChoice) => {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			toast(
				message.includes("switch_session")
					? "Session switching requires a Pi version with RPC switch_session support."
	...
	onMount(() => {
		...
		if (themeWarning)
			queueMicrotask(() => toast(themeWarning, "warning", 10_000));
		if (!isUpdateCheckDisabled(process.env)) {
			void checkForUpdates(
				defaultUpdateCheckConfig(
					...
					(version) => {
						toast(
							`A newer PiTTy release is available (${version}). Run \`pitty upgrade\`.`,
							"info",
							7000,
						);
					},
		...
		if (missingIntegrations.length) {
			queueMicrotask(() =>
				toast(
					`Optional Pi packages not detected: ${missingIntegrations.join(", ")}. PiTTy will continue without them.`,
					"info",
					7000,
				),
			);
		...
		void (async () => {
			try {
				...
				props.logger.info("ui.history_loaded", {
					...
					assistantItems: historyItems.filter(
						(item) => item.kind === "assistant",
					).length,
					toolItems: historyItems.filter((item) => item.kind === "tool").length,
					textChars: historyItems.reduce(
						(sum, item) =>
							sum +
							("text" in item
								? item.text.length
		...
		clockTimer = setInterval(() => setClockNow(Date.now()), 1000);
		...
		onCleanup(() => {
			for (const runtime of provisionalRuntimes) {
				runtime.messageUpdates.dispose();
				runtime.coalescer.flush();
				void runtime.client.stop();
			}
			provisionalRuntimes.clear();
	...
	const sendSubagentSteer = (message: string) => {
		...
		try {
			...
			touch();
			props.logger.info("subagent.steer_requested", {
				runId: target.run.runId,
			...
			props.logger.error("subagent.steer_failed", {
				runId: target.run.runId,
				index: target.stepIndex,
				error,
			});
	...
	useKeyboard((event) => {
		...
		if (suggestions.length && !inspectSubagent()) {
			...
			if (event.name === "tab" && !event.shift) {
				event.preventDefault();
				event.stopPropagation();
				const command = selectCommandChoice(
					suggestions,
		...
		const navigation = subagentNavigationAction(
			inspectSubagent(),
			event.name,
			event,
		);
		...
		if (keyIs(event, "end", { ctrl: true })) {
			event.preventDefault();
			event.stopPropagation();
			(inspectSubagent() ? subagentScroll : scroll)?.scrollTo(
		...
		if (keyIs(event, "i", { ctrl: true })) {
			...
			if (decision === "warning")
				return toast("No subagent selected", "warning");
			if (decision === "choose") {
							...
							<Show when={showWorkingIndicator()}>
								...
								>
									<text fg={colors.green} attributes={1}>
										{spinnerFrames[spinnerIndex()] ?? "◐"} Working…
									</text>
								</box>
							</Show>
						</scrollbox>
			...
			>
				...
				<text fg={colors.subtle}>
					{globalFooterHint(
						dimensions().width,
						inspectSubagent(),
						availableSubagentTargets().length,
					)}
				</text>
			</box>
			<For each={toasts()}>
	...
	return (
		<>
			{createDynamic(
				() => (themeController.revision() % 2 === 0 ? EvenTheme : OddTheme),
				{},
			)}
			<Show when={settingsRoute() === "root"}>
				<SettingsHub
					...
					onSelected={setSettingsSelectedId}
					onClose={closeSettings}
				/>
			</Show>
			<Show when={settingsRoute() === "mcp"}>
				<McpSettings
					adapterState={mcpInstallState}
					...
					error={mcpError}
					view={mcpView}
					onViewChange={setMcpView}
					onInstall={installMcpAdapter}
					onScope={(scope) => {
						setMcpScope(scope);
						openMcpSettings();
					}}
					onAdd={prepareMcpMutation}
					onDelete={deleteMcpServer}
					onConfirmPreview={confirmMcpMutation}
					onCancelPreview={() => {
						setMcpPreview(undefined);
						openMcpSettings();
					}}
					onRetryActivation={() => void activateMcp()}
					onBack={() => setSettingsRoute(settingsBackRoute("mcp"))}
				/>
			</Show>
			...
			<Show when={settingsRoute() === "thinking"}>
				<ThinkingSelector
					...
					{...(settingsErrors().thinking
						? { error: settingsErrors().thinking }
						: {})}
					onSelect={(level) => void applySettingsThinking(level)}
					onCancel={() => setSettingsRoute("root")}
				/>
			</Show>
			...
			<Show when={settingsRoute() === "session"}>
				<SessionSettings
					...
					onSwitch={() => {
						if (!streaming()) {
							setSettingsRoute("session-switch");
							openSessionSelector();
						}
					}}
					onRename={() => setSettingsRoute("session-rename")}
					onCancel={() => setSettingsRoute("root")}
				/>
			</Show>
			<Show when={settingsRoute() === "session-rename"}>
				<SessionRename
					initialName={sessionState()?.sessionName ?? ""}
					busy={() => settingsBusy() === "rename"}
					error={() => settingsErrors().rename}
					onConfirm={(name) => void applySettingsRename(name)}
					onCancel={() => setSettingsRoute("session")}
				/>
			</Show>
			...
			<Show when={notificationDetailId()}>
				<NotificationDialog
					record={() =>
						notificationHistory().find(
							(record) => record.id === notificationDetailId(),
						)
					}
