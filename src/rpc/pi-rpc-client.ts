...
export class PiRpcClient extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    child.on("exit", (code, signal) => {
      ...
      this.options.logger?.error("pi.exit", { code, signal, stopping: this.stopping, stderrTail: this.stderrBuffer.slice(-8_000) });
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once("exit", (code, signal) => {
        clearTimeout(timer);
        reject(new Error(`Pi exited during startup (code=${code}, signal=${signal ?? "none"}).`));
      });
    });
  }
  ...
  async abort(): Promise<void> {
    try {
      ...
      if (error instanceof Error && error.message === "Timed out waiting for Pi response to abort.") {
  ...
  async getMessages(): Promise<unknown[]> {
    const data = this.data<{ messages: unknown[] }>(await this.request({ type: "get_messages" }));
  ...
  async getAvailableModels(): Promise<unknown[]> {
    const data = this.data<{ models: unknown[] }>(await this.request({ type: "get_available_models" }));
    return data.models;
  }
  ...
  async getCommands(): Promise<Array<{ name: string; description?: string; source: string }>> {
    const data = this.data<{ commands: Array<{ name: string; description?: string; source: string }> }>(await this.request({ type: "get_commands" }));
  ...
  private request(command: RpcCommand, timeoutMsOverride?: number): Promise<RpcResponse> {
    ...
    return new Promise<RpcResponse>((resolve, reject) => {
      ...
      this.pending.set(id, {
        ...
        reject: (error) => {
          this.options.logger?.error("rpc.request_failed", { id, command: command.type, durationMs: Math.round(performance.now() - startedAt), error });
          reject(error);
        },
        timer,
      });
  ...
  private consumeStdout(chunk: string): void {
    ...
    while (true) {
      ...
      try {
        ...
        const error = new Error(`Non-JSON Pi RPC output: ${line.slice(0, 300)}`);
        this.options.logger?.error("rpc.protocol_error", { line: line.slice(0, 2_000) });
