...
export function selectCommandChoice(
	commands: CommandChoice[],
	index: number,
): CommandChoice | undefined {
...
export function filterCommandChoices(
	...
	// Only offer suggestions while the caret is at the end of the prompt so
	// ArrowUp/Down can browse prompt history when the caret is elsewhere.
	if (cursorOffset !== undefined && cursorOffset !== input.length) return [];
...
export function CommandSuggestions(props: CommandSuggestionsProps) {
	...
	const ensureSelectedVisible = () => {
		...
		if (height <= 0 || (rowTop >= scrollTop && rowBottom <= scrollTop + height))
			return;
					...
					{(command, index) => {
						...
						return (
							<box
								...
								flexDirection="row"
								backgroundColor={
									selected() ? colors.selection : colors.panelRaised
								}
								onMouseDown={(event) => {
									event.preventDefault();
									event.stopPropagation();
							...
							>
								<text
									width={24}
									fg={selected() ? colors.textBright : colors.cyan}
									wrapMode="none"
								>
									/{command.name}
								</text>
								<text
									...
									wrapMode="none"
								>
									{clip(command.description ?? command.source ?? "")}
								</text>
							</box>
						);
					}}
