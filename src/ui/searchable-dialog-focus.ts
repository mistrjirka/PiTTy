...
export function handleSearchableDialogCancel(
	event: KeyEvent,
	onCancel: () => void,
): boolean {
...
export function createSearchableDialogFocus(
	options: SearchableDialogFocusOptions,
): SearchableDialogFocusController {
	const [focusTarget, setFocusTarget] =
		createSignal<SearchableDialogFocus>("search");
	let active: SearchableDialogFocus = "search";
	const focusSearch = () => {
		active = "search";
		setFocusTarget("search");
		options.getSearch()?.focus();
	};
	const focusList = () => {
	...
	onMount(() => setTimeout(focusSearch, 0));
	return {
		focusTarget,
		activeFocus: () => active,
		focusSearch,
		...
		onKeyDown: (event) => {
			...
			if (
				active === "search" &&
				(event.name === "down" || event.name === "up") &&
				options.getListLength()
			) {
				event.preventDefault();
				event.stopPropagation();
				focusList();
