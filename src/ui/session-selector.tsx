...
type SessionSelectorFocus = "search" | "list";
...
export function SessionSelector(props: SessionSelectorProps) {
  ...
  const [focusTarget, setFocusTarget] = createSignal<SessionSelectorFocus>("search");
  let activeFocus: SessionSelectorFocus = "search";
  const setActiveFocus = (next: SessionSelectorFocus) => {
    activeFocus = next;
    setFocusTarget(next);
  };
  ...
  useKeyboard((event) => {
    ...
    if (event.name === "tab") {
      event.preventDefault();
      event.stopPropagation();
      if (activeFocus === "search") focusList();
      else focusSearch();
      return;
    }
    if (activeFocus === "search" && (event.name === "down" || event.name === "up")) {
      event.preventDefault();
      event.stopPropagation();
      focusList();
      if (event.name === "up") select?.setSelectedIndex(Math.max(0, choices().length - 1));
      return;
    }
    if ((event.name === "enter" || event.name === "return") && activeFocus === "search") {
  ...
  return (
    <box position="absolute" left="12%" right="12%" top="8%" bottom={5} flexDirection="column" backgroundColor={colors.panelRaised} border borderColor={colors.borderStrong} padding={1} zIndex={150}>
      ...
      {props.pending ? <box flexDirection="column"><text fg={props.streaming ? colors.yellow : colors.green}>{props.streaming ? `Pi is streaming. Switch to “${props.pending.name}”?` : `Pi has settled. Switch to “${props.pending.name}”?`}</text><text fg={colors.subtle}>Enter/Y confirm · N decline · Esc close</text></box> : null}
      <textarea ref={(value) => { search = value; }} focused={focusTarget() === "search" && !props.pending} height={1} minHeight={1} maxHeight={1} placeholder="Search sessions…" backgroundColor={colors.panel} focusedBackgroundColor={colors.panel} textColor={colors.textBright} placeholderColor={colors.muted} onKeyDown={cancelOnEscape} onContentChange={() => { setQuery(search?.plainText ?? ""); queueMicrotask(() => select?.setSelectedIndex(0)); }} />
      <text fg={colors.subtle}>Tab list · ↑/↓ move · Enter select · Esc close</text>
      {props.switching ? <text fg={colors.muted}>Switching session…</text> : props.state.kind === "loading" ? <text fg={colors.muted}>Loading sessions…{props.state.progress ? ` ${props.state.progress.loaded}/${props.state.progress.total}` : ""}</text> : props.state.kind === "error" ? <box flexDirection="column"><text fg={colors.red}>Unable to discover sessions: {props.state.error}</text>{props.onRetry ? <text fg={colors.yellow} onMouseDown={(event) => { event.preventDefault(); props.onRetry?.(); }}>Retry</text> : null}</box> : choices().length === 0 ? <text fg={colors.yellow}>No resumable sessions in this directory.</text> : <select ref={(value) => { select = value; }} options={options()} selectedIndex={0} focused={focusTarget() === "list" && !props.pending} height={Math.min(18, Math.max(5, options().length * 2))} backgroundColor={colors.panelRaised} focusedBackgroundColor={colors.panelRaised} textColor={colors.text} focusedTextColor={colors.text} selectedBackgroundColor={colors.selection} selectedTextColor={colors.textBright} descriptionColor={colors.muted} selectedDescriptionColor={colors.text} showScrollIndicator wrapSelection onKeyDown={cancelOnEscape} onSelect={(_index, option) => { if (option?.value) props.onSelect(option.value); }} />}
    </box>
  );
}
