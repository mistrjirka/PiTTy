...
export function ExtensionDialog(props: {
  ...
  const title = createMemo(() => "title" in props.request ? props.request.title : "Pi extension");
  const options = createMemo(() => props.request.method === "select" ? props.request.options.map((name) => ({ name, description: "", value: name })) : []);
  return (
    <box
      position="absolute"
      left="15%"
      ...
      zIndex={100}
    >
      <text fg={colors.textBright} attributes={1}>{title()}</text>
      <Show when={props.request.method === "confirm"}>
        <text fg={colors.text} wrapMode="word">{props.request.method === "confirm" ? props.request.message : ""}</text>
      ...
      <Show when={props.request.method === "select"}>
        <select
          ...
          onSelect={(_index, option) => option && props.onValue(String(option.value ?? option.name))}
        />
      </Show>
      <Show when={props.request.method === "input" || props.request.method === "editor"}>
        <textarea
          ...
          focused
          initialValue={props.request.method === "editor" ? props.request.prefill ?? "" : ""}
          placeholder={props.request.method === "input" ? props.request.placeholder ?? "" : ""}
