...
import { subagentTreeRows, type SubagentTarget } from "../subagents/targets.ts";
...
function description(target: SubagentTarget): string {
  ...
  const group = target.active ? "ACTIVE" : target.state === "unresponsive" ? "UNRESPONSIVE" : "FINISHED";
...
export function SubagentSelectorDialog(props: {
  ...
  const options = createMemo(() => rows().map((row) => ({
    name: `${row.depth > 0 ? `${"  ".repeat(Math.min(row.depth, 4))}↳ ` : ""}${row.target.active ? "●" : "○"} ${row.target.label}`,
